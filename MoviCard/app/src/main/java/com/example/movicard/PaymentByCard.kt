...
class PaymentByCard : AppCompatActivity() {
    ...
    private fun createPaymentIntent() {
        ...
        val request = Request.Builder()
            .url("http://192.168.56.244:3000/create-payment-intent") // Backend local para pruebas
            .post(requestBody)
            .build()
    ...
    private fun obtenerUrlFactura(paymentIntentId: String, callback: (String) -> Unit) {
        val client = OkHttpClient()
        val request = Request.Builder()
            .url("http://192.168.56.244:3000/get-receipt-url/$paymentIntentId") // 🔹 Reemplaza con la IP de tu backend si pruebas en un móvil
            .get()
            .build()
